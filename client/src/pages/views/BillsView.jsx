...
export default function BillsView({ onRefresh }) {
  ...
  const [formData, setFormData] = useState({ name: '', amount: '', dueDay: '', isAutoPay: false, alreadyPaid: false });
  ...
  const openAddForm = () => {
    setEditingBill(null);
    setFormData({ name: '', amount: '', dueDay: '', isAutoPay: false, alreadyPaid: false });
    setFormError('');
    setIsFormOpen(true);
  };
  ...
  const openEditForm = (bill) => {
    setEditingBill(bill);
    setFormData({ name: bill.name, amount: bill.amount.toString(), dueDay: bill.dueDay.toString(), isAutoPay: bill.isAutoPay, alreadyPaid: false });
    setFormError('');
    setIsFormOpen(true);
  };
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAddForm} className="btn-primary text-sm">+ Add Bill</button>
      </div>
            ...
            <div key={bill.id} className={`card transition-all ${!bill.isActive ? 'opacity-50' : 'hover:shadow-md'}`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full flex-shrink-0 ${bill.isActive ? 'bg-green-500' : 'bg-gray-300'}`} />
                  <div>
                    <p className="font-medium text-gray-900">{bill.name}</p>
                    <p className="text-sm text-gray-500">Due on the {ordinal(bill.dueDay)} • {bill.isAutoPay ? 'Auto-pay' : 'Manual'}</p>
                  </div>
                </div>
      ...
      {bills.length > 0 && (
        <div className="mt-6 card bg-gray-50">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xl font-bold text-gray-900">{formatCurrency(bills.filter(b => b.isActive).reduce((sum, b) => sum + b.amount, 0))}</span>
          </div>
        </div>
      )}
